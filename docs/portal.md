...
MTS Dimension Warp is a fork of [Dimension Warp](https://mods.factorio.com/mod/dimension-warp) by Guillaume ("Anakhon"), rebuilt on the MTS `mts-v1` API. Everything that makes the warp feel the way it does — the platform, the dimension techs, the harvesters, the art and the sound — is Anakhon's work. Dimension Warp is BSD 3-Clause; its licence text is retained verbatim in [LICENSE.dimension-warp](https://github.com/bits-orio/mts-dimension-warp/blob/main/LICENSE.dimension-warp) and the provenance is recorded in [NOTICE](https://github.com/bits-orio/mts-dimension-warp/blob/main/NOTICE). Per the BSD licence's third clause, the names "Dimension Warp", "Guillaume" and "Anakhon" are not used to endorse or promote this fork.
